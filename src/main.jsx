...
function Leader(){
 ...
 async function submit(){if(!period)return;const rows=Object.values(entries).filter(x=>x.status==='draft');if(!rows.length){setMessage('Tidak ada data draft untuk dikirim.');return}if(!confirm(`Submit ${rows.length} entri untuk ${identity.group_name}?`))return;setBusy(true);const {error}=await supabase.rpc('submit_ef_for_nik',{p_nik:identity.nik,p_period_id:period.id});if(error)setErr(error.message);else{setMessage('Data berhasil disubmit dan dikunci.');await reloadEntries()}setBusy(false)}
